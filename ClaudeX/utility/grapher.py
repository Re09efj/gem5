...
# ─────────────────────────────────────────────
def generate_core_stats_graph(csv_path: str, num_nodes: int,
                               big_cpn: int, sml_cpn: int) -> None:
    ...
    for node_id in range(num_nodes):
        for cpu_id in range(cores_per_node):
            label     = f"N{node_id}-{'Big' if cpu_id < big_cpn else 'Small'}{cpu_id}"
    ...
    sns.barplot(x="Core", y="CPI", data=df_cpi,
                hue="CoreType", palette=type_palette,
                ax=axes[0], legend=True, dodge=False)
    axes[0].set_title("CPI per Core (Lower is Better)", fontsize=14, fontweight="bold")
    ...
    sns.barplot(x="Core", y="NumCycles", data=df_cycles,
                hue="CoreType", palette=type_palette,
                ax=axes[1], legend=True, dodge=False)
...
# 2. 単一実験 NUMA アクセス比較
# ─────────────────────────────────────────────
def generate_numa_graph(output_dir: str, node_stats: dict,
                        preset_name: str, cpu_map: list,
                        num_threads: int,
                        workload: str = "CG", bench_class: str = "?") -> None:
    if not node_stats:
        return
    ...
    bars_w = axes[0].bar(x, writes, color=colors_write, label="Writes",
                         bottom=reads, width=0.5)
