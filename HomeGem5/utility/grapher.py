...
def generate_core_stats_graph(csv_path,nn,bcpn,scpn):
        ...
        for cpu_id in range(CORES_PER_NODE):
            ...
            if not cpi_row.empty:
                cpi_data.append({
                    ...
                    "NodeID":  node_id,
                    "CoreType": "Big" if cpu_id < BIG_CORES_PER_NODE else "Small"
                })
    ...
    if df_cpi.empty or df_cycles.empty:
        print("⚠️ 警告: グラフに必要なCPUデータ(cpi/numCycles)が見つかりませんでした。")
        return
    ...
    output_graph_path = os.path.join(os.path.dirname(csv_path), "core_stats_summary.png")
