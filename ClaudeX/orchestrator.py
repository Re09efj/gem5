...
from utility.cpu_affinity  import get_cpu_map, compile_strategy
...
def _compile_workload(workload: str) -> dict[str, str]:
    ...
    for strategy in STRATEGIES_TO_RUN:
        ...
        binaries[strategy] = compile_strategy(strategy, cpu_map, workload, BENCH_CLASS)
    return binaries
...
def _run_gem5(binary: str, workload: str, strategy: str, run_id: str) -> str:
    output_dir = os.path.join(
        OUTPUT_BASE, f"{workload}_{BENCH_CLASS}_{strategy}_{NUM_THREADS}TH_{run_id}"
    )
    os.makedirs(output_dir, exist_ok=True)
    cmd = [
        GEM5, "-d", output_dir, MYEXP,
        f"--strategy={strategy}",
...
def _run_all_gem5(all_binaries: dict, run_id: str) -> dict[tuple[str, str], str]:
...
def _collect_results(output_dirs: dict) -> dict[str, list]:
    ...
    for wl in WORKLOADS:
        for strategy in STRATEGIES_TO_RUN:
            ...
            save_affinity_config(output_dir, strategy, wl, BENCH_CLASS,
                                 cpu_map, NUM_THREADS, NUM_NODES, BIG_CPN, SML_CPN)
            generate_numa_graph(output_dir, node_stats, strategy,
                                cpu_map, NUM_THREADS, wl, BENCH_CLASS)
            results[wl].append({
                "strategy":   strategy,
...
def _print_summary(results: dict, run_id: str) -> None:
    ...
    for wl in WORKLOADS:
        for r in results[wl]:
            ...
            cmap  = r["cpu_map"]
            cores = " ".join(
                f"T{t}→{_core_label(cmap[t])}" for t in range(min(NUM_THREADS, 4))
            )
            if NUM_THREADS > 4:
                cores += " ..."
...
def main():
    ...
    print(f"  ワークロード: {WORKLOADS}  クラス={BENCH_CLASS}  スレッド数={NUM_THREADS}")
